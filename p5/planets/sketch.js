...
class Planet {
  ...
  output() {
    ...
    if (this.history.length > store) this.history.pop();

    for (let i = this.history.length - 1; i >= 0; i--) {
      let bufr = 20;
      fill(
        ((this.cr - bufr) * (store - i)) / store + bufr,
        ((this.cg - bufr) * (store - i)) / store + bufr,
        ((this.cb - bufr) * (store - i)) / store + bufr
      );
...
class SystemOfPlanets {
  ...
  refresh() {
    ...
    for (let k = 0; k < this.planets.length; k++) this.planets[k].refreshNew();
  }
   ...
   */
  output() {
    for (let i = 0; i < this.planets.length; i++) this.planets[i].output();
  }
}
...
function timeToString(secs) {
  ...
  let ans = "";
  if (hr > 0) ans = hr + "h " + (min % 60) + "min " + (secs % 60) + "s";
  else if (min > 0) ans = min + "min " + (secs % 60) + "s";
  else ans = secs + "s";
  return ans;
}
...
function draw() {
  ...
  for (let i = 0; i < FASTER * MULT; i++) solar.refresh();
